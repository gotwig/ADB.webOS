...
void lv_draw_sdl_texture_cache_remove_src(lv_draw_sdl_ctx_t * ctx, const void * src, int32_t frame_id)
{
    size_t key_size;
    lv_draw_sdl_cache_key_head_img_t * key = lv_draw_sdl_texture_img_key_create(src, frame_id, &key_size);
    if(key) {
        lv_lru_remove(ctx->internals->texture_cache, key, key_size);
        SDL_free(key);
    }
}
