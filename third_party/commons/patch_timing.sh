...
int session_worker(session_t *session) {
    ...
    // Step 1: Push server
    snprintf(cmd, sizeof(cmd), "%s push /media/developer/apps/usr/palm/applications/com.limelight.webos/scrcpy-server.jar /data/local/tmp/scrcpy-server.jar 2>&1 | tee /tmp/adb_push.log", adb);
    if (system(cmd) != 0) {
    ...
    // Step 2: Forward port
    snprintf(cmd, sizeof(cmd), "%s forward tcp:27183 localabstract:scrcpy 2>&1 | tee /tmp/adb_forward.log", adb);
    if (system(cmd) != 0) {
        session_interrupt(session, false, STREAMING_INTERRUPT_ERROR);
    ...
    pid_t pid = fork();
    if (pid == 0) {
        execlp(adb, adb, "shell", "CLASSPATH=/data/local/tmp/scrcpy-server.jar", "app_process", "/", "com.genymobile.scrcpy.Server", "3.1", "tunnel_forward=true", "audio=false", "control=false", "max_size=1280", "raw_stream=true", "video_bit_rate=3000000", "max_fps=30", "i_frame_interval=2", NULL);
        exit(1);
    } else if (pid > 0) {
        adb_server_pid = pid;
    }
    ...
    // Step 4: Wait for server to be ready before connecting (just like setTimeout in index.js)
    int retries = 20; // Try for up to 10 seconds total (500ms * 20)
    SDL_Delay(1000);  // Initial 1s delay to let server boot up
...
thread_cleanup:
    ...
    if (adb_server_pid > 0) {
        // Stop adb shell process
        kill(adb_server_pid, SIGTERM);
        adb_server_pid = 0;
        // Clean up port forwarding
        snprintf(cmd, sizeof(cmd), "%s forward --remove tcp:27183", adb);
        system(cmd);
    }
