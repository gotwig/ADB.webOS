...
#else
    int (*setMediaVideoData)(long acbId, const char *payload, long *taskId) = (void *) &AcbAPI_setMediaVideoData;
    setMediaVideoData(resource->acbId, modified_info, NULL);
#endif
...
bool StarfishResourcePostLoad(StarfishResource *resource, const SS4S_VideoInfo *info) {
    ...
    if (info->width > 0 && info->height > 0) {
        if (info->width < info->height) {
            // Video is portrait -> fit height, pillarbox sides
            double video_ratio = (double)info->width / info->height;
            int h = 1080;
            int w = (int)(1080.0 * video_ratio);
            int x = (1920 - w) / 2;
            int y = 0;
            AcbAPI_setDisplayWindow(resource->acbId, x, y, w, h, false, &resource->taskId);
        } else {
            // Video is landscape -> stretch/scale to fill the TV screen completely (16:9)
            AcbAPI_setDisplayWindow(resource->acbId, 0, 0, 1920, 1080, true, &resource->taskId);
        }
    } else {
