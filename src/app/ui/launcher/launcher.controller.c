...
static void launcher_view_init(lv_fragment_t *self, lv_obj_t *view) {
    ...
    lv_obj_add_event_cb(fragment->scrcpy_btn, start_scrcpy, LV_EVENT_CLICKED, fragment);
...
static void select_pc(launcher_fragment_t *controller, const uuidstr_t *uuid, bool refocus) {
    if (uuid) {
        if (controller->welcome_cnt) {
            lv_obj_add_flag(controller->welcome_cnt, LV_OBJ_FLAG_HIDDEN);
        }
        ...
        if (controller->welcome_cnt) {
            lv_obj_clear_flag(controller->welcome_cnt, LV_OBJ_FLAG_HIDDEN);
        }
    }
